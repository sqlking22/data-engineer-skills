...
> **比例说明**：下方 60%/25% 为**通用软件金字塔**原则示意（底层单元测试最多）。本套件面向**数仓测试**，推荐比例见 [test-standards.md](test-standards.md) 的数仓测试金字塔（数据质量检查占更大比重）——两者不冲突，前者是原则，后者是数仓场景的落地。

```python
# 正确：金字塔结构（示意：底层单元测试最多）
# 60% 单元测试（快、隔离、易调试）
