...
| 基于模型测试 | `/integration-test --from-model` | modeling_package.yaml | [上游输入](#上游输入) |
...
测试通过后，`test_package.yaml` 作为上线交付物，由人工或团队既有的发布流程（本套件不含独立部署模块）核验后发布：

```
## 测试通过后的上线检查

# 核验 outputs/test_package.yaml 的 summary 字段：
- overall_status == "passed"      # 所有测试必须通过
- block_deployment == false       # 未阻塞部署
- 关键路径测试全部通过
- 覆盖率 > 80%（核心表建议 > 90%）
...
> 说明：本套件未包含独立的"部署/发布"模块。测试包产出后，发布动作由团队既有的发布流程（DataWorks 发布、DolphinScheduler 任务上线等）承接。
...
> 📖 **完整最佳实践指南**：[references/best-practices.md](references/best-practices.md) — 包含测试金字塔、Fixture 策略、性能基线、CI/CD 集成。

### 0. 速查卡片

| 卡片 | 核心原则 |
|------|---------|
| 🔺 测试金字塔 | 单元测试 > 集成测试 > UI 测试 |
| 🎯 Fixture 独立 | 每个测试用独立数据集 |
| 💬 断言信息丰富 | 失败时能快速定位问题 |
| 📏 容忍度合理 | 业务场景决定容忍度 |
| ⏱ 性能基线 | 关键查询有性能基线，回归时自动对比 |
| 🔒 数据隔离 | 测试数据不污染生产 |
| 🚀 CI 集成 | 测试纳入 CI/CD |
| 📊 覆盖率监控 | 关键表覆盖率 > 90% |
...
**自动传递**：
- dq-assistant 输出: `quality_rules` (质量规则配置)
- test-engineer 输入: 将质量规则转化为pytest断言
