...
1. 【中优先级】处理手机号重复问题 (23条)
   ...
   查看SQL: SELECT phone, COUNT(*), GROUP_CONCAT(id)
...
**输入**：
```
/dq-doc 生成users表的完整数据字典，包含样例数据、血缘关系、质量评分
```
...
|------|--------|----------|------|--------|------|------|--------|----------|
| 1 | id | BIGINT | NO | AUTO_INCREMENT | PK | ✅ | 用户ID | 主键，BIGINT AUTO_INCREMENT（值唯一、非顺序递增） |
...
> AnalyticDB MySQL 不支持 UNIQUE KEY 与独立 CREATE INDEX；索引通过建表语句内联 INDEX / CLUSTERED KEY 定义，唯一性由数据质量规则保障。

| 索引名 | 类型 | 字段 | 说明 |
|--------|------|------|------|
| pk_users | PRIMARY KEY | id | 主键（含分布键/分区键） |
| idx_users_email | INDEX | email | 邮箱筛选（内联） |
| idx_users_username | INDEX | username | 用户名筛选（内联） |
| idx_users_phone | INDEX | phone | 手机号筛选（内联） |
| idx_users_status | INDEX | status | 状态筛选（内联） |
| idx_users_created | INDEX | created_at | 时间筛选（内联） |
| ck_users_status | CLUSTERED KEY | status | 聚集索引，按状态聚簇存储 |
...
| UNIQUE 规则 | 数据质量规则 | email/username/phone | 唯一性由 DQ 规则校验（ADB 不支持 UNIQUE KEY） |
| 枚举规则 | 数据质量规则 | status | 状态枚举由 DQ 规则校验（ADB 不支持 CHECK 约束） |
...
```bash
# 更新数据字典
/dq-doc users,orders,products --include-quality
