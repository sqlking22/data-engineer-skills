...
name: dq-assistant
description: |
  数据质量检查助手 - 端到端数据质量管理工作流。包含规则生成、质量检查、数据字典三大核心功能。
  当用户需要建立数据质量监控、执行质量检查、生成数据字典时触发。
  触发词：数据质量、质量检查、数据字典、字段血缘文档、质量监控。
---
...
| 3 | /dq-doc | general-purpose | 生成数据字典和血缘文档 |

**输入**: modeling_package.yaml / sql_package.yaml（可选）
...
| 生成质量规则时 | [references/dq-rule-gen.md](references/dq-rule-gen.md) | 规则生成器详解、规则模板 | /dq-rule-gen 命令参考 |
| 执行质量检查时 | [references/dq-check.md](references/dq-check.md) | 检查执行器详解、检查模式 | /dq-check 命令参考 |
| **最佳实践指南** | [references/best-practices.md](references/best-practices.md) | **规则反模式、阈值设置、告警分级、踩坑教训** |
...
| 生成数据字典 | `/dq-doc [表名]` | [功能3](#功能3数据字典文档生成器-dq-doc) |
...
# 阶段3: 生成数据字典
/dq-doc 生成orders表的完整数据字典，包含样例数据和质量评分
```
...
```bash
# 方式1: 显式引用上游包
/dq-rule-gen 基于 modeling_package.yaml 为所有目标表生成质量规则
...
> 📖 **完整最佳实践指南**：[references/best-practices.md](references/best-practices.md) — 包含规则反模式、阈值设置、告警分级、踩坑教训。

### 0. 速查卡片

| 卡片 | 核心原则 |
|------|---------|
| 🛡️ 预防胜于检查 | 在 ETL 源头约束，比事后检查更有效 |
| 📊 分级管理 | 高/中/低优先级对应不同严重度和响应 |
| ⏱ 分层监控 | 不同层级（核心/边缘）采用不同检查频率 |
| 📈 指标可量化 | 质量评分可计算、可对比、可追踪 |
| 🚨 告警分级 | 不同严重度对应不同渠道和时效 |
| 🔄 持续改进 | 规则随业务变化而调整 |
| 👤 Owner 明确 | 每个表/规则必须有明确负责人 |
| 💡 可解释性 | 异常原因可追溯 |
