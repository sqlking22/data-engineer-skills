...
name: model-design
description: |
  数据模型设计助手 - 基于 Kimball 维度建模方法论（事实表/维度表/SCD策略）进行表级设计。
  本助手专注于 Kimball 部分的"自底向上设计"，与 onedata-methodology（自顶向下规划）配合使用。
...
> **前置依赖**：本助手专注于 Kimball 维度建模（自底向上设计）。如果还没有 OneData 的数据域划分和总线矩阵，请先使用 [onedata-methodology.md](onedata-methodology.md) 完成自顶向下的规划。

1. **消费上游 OneData 产物** - 读取总线矩阵、指标字典、数据域划分
2. **业务分析** - 理解业务流程和分析需求
3. **粒度确定** - 确定事实表的粒度（最细级别）
4. **维度识别** - 识别相关的业务维度（从总线矩阵的"✓"单元格提取）
5. **度量确定** - 确定可累加的度量值（从指标字典的原子指标提取）
6. **模型生成** - 输出完整的数据模型设计
...
> **命名约定**：事实表使用层级前缀（`dwd`/`dws`/`ads`），**不使用 `fact_`/`fct_` 前缀**；仅维度表使用 `dim_` 前缀。详见 `docs/数据仓库规范体系.md`。下文示例事实表均以 `dwd_` 开头。

### dwd_order_item（订单项事实表）
...
| 目标表 | 源表 | 转换逻辑 |
|--------|------|----------|
| dwd_order_item | ods_orders + ods_order_items | JOIN关联，金额计算 |
