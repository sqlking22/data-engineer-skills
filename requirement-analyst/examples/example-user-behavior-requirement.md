...
> ⚠️ **平台范围说明**：本示例涉及 Flink/Spark/ClickHouse/Redis/HBase 等实时技术栈，仅作**需求分析思维演示**。本套件的 SQL 生成（`/sql-gen`）仅支持 **AnalyticDB MySQL** 和 **MaxCompute** 离线数仓；如需落地实时方案，需另行评估实时计算平台支持。本示例中生成的下游 SQL 命令（如"生成 Flink SQL"）在当前套件中**不会产出实际可执行代码**。
...
```bash
# Step 1: 实时流开发（通过 DataWorks/DolphinScheduler 调度 Flink 任务）
/sql-gen 生成Flink实时处理SQL：
- 源: Kafka (user_events topic)
- 处理: JSON解析、窗口聚合、用户关联
- 输出: Redis(实时指标) + ClickHouse(明细)

# Step 2: 用户画像批处理（通过 DataWorks/DolphinScheduler 调度 Spark 任务）
/sql-gen 生成Spark用户画像SQL：
- 源: ClickHouse事件明细
