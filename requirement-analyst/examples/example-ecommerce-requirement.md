...
parse_result:
  ...
  inferred:
    ...
    tech_stack: "推荐 ADB MySQL + DataWorks + DolphinScheduler"
```
...
```bash
/sql-gen 生成订单同步SQL（增量UPSERT）：

配置：
- 名称: order_sync
- 源: MySQL (orders, order_items)
- 目标: AnalyticDB MySQL (fct_order_items)
- 策略: 增量UPSERT，按 updated_at 识别变更
- 调度: 每日凌晨2点（通过 DolphinScheduler/DataWorks 调度）
...
3. 执行维度表DDL
4. 运行维度同步（初始化）
5. /sql-gen → 生成事实表抽取SQL
6. /sql-gen → 生成事实表同步SQL
7. /dq-rule-gen → 生成质量规则
8. 部署Pipeline并测试
```
```
...
requirement_package:
  ...
  technical:
    preferred_stack: ["AnalyticDB MySQL", "DataWorks", "DolphinScheduler"]
    estimated_volume:
      daily: "100万订单"
      total: "约10亿历史订单"
