...
requirement_package:
  ...
  technical:
    preferred_stack: ["AnalyticDB MySQL", "MaxCompute", "DataWorks"]
...
> 📖 **完整最佳实践指南**：[references/best-practices.md](references/best-practices.md) — 包含反模式、避坑、需求描述模板、经验教训。

### 0. 速查卡片

| 卡片 | 核心原则 |
|------|---------|
| 🎯 业务驱动 | 需求聚焦业务价值，不提前限定技术方案 |
| 📊 口径明确 | 每个指标必须有唯一、清晰的计算公式 |
| 🔍 粒度先于指标 | 先确定数据粒度，再设计指标 |
| 🚧 边界清晰 | 明确"做什么"和"不做什么"，避免范围蔓延 |
| ✅ 可验证性 | 每个需求必须有可验证的验收标准 |
| 📝 书面留痕 | 重要澄清必须有书面确认，不依赖口头约定 |
| 🔄 变更受控 | 任何变更都需评估影响并经过审批 |
