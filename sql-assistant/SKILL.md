...
description: |
  ...
  支持阿里云数据库：AnalyticDB MySQL、MaxCompute。
...
| **最佳实践指南** | [references/best-practices.md](references/best-practices.md) | **SQL 写法正反例、ADB vs MaxCompute 差异、性能调优** |
| 使用示例 | `examples/` 目录 | 典型场景的输入输出示例 |
| ADB示例 | [examples/example-adb-ddl.md](examples/example-adb-ddl.md) | AnalyticDB DDL示例 |
| ADB ETL示例 | [examples/example-adb-etl.md](examples/example-adb-etl.md) | AnalyticDB ETL示例 |
| ADB分析示例 | [examples/example-adb-analytics.md](examples/example-adb-analytics.md) | AnalyticDB分析查询示例 |

## 支持的数据库

| 数据库 | 类型 | 支持程度 | 特殊说明 |
|--------|------|---------|---------|
| **MaxCompute** | 阿里云大数据 | ✅ 完整支持 | 海量数据处理、分区表、MAPJOIN |
| **AnalyticDB MySQL** | 阿里云数仓 | ✅ 完整支持 | 数仓底座、分布式表、聚簇索引、分区策略 |
...
**示例**：
```
/sql-gen 使用AnalyticDB MySQL语法，查询过去7天每天的新增用户数，
         要求显示日期、新增用户数、环比增长百分比

/sql-gen 使用AnalyticDB MySQL，创建订单事实表，按月分区
         包含订单ID、用户ID、订单时间、金额字段
```
...
- ADB MySQL：包含DISTRIBUTED BY和PARTITION BY定义
...
**支持的数据库**：
- AnalyticDB MySQL (EXPLAIN, 执行计划分析)
- MaxCompute (执行计划、LogView)
...
sql_package:
  ...
  metadata:
    ...
    sql_dialect: "MaxCompute|AnalyticDB MySQL"
...
```markdown
【强制】SQL 生成前置检查清单：
- [ ] **数据库类型已确认**（MaxCompute / AnalyticDB MySQL）
  - 如果用户未指定 → 必须主动询问
  - **AnalyticDB MySQL特有检查**：
    - 确认是否为阿里云数据仓库场景
    - 确认分布键选择（高基数字段如order_id/user_id）
    - 确认分区策略（按月/按日）
    - 主键必须包含分区键
- [ ] **表结构信息已知**（有DDL或样本数据）或已在对话中提供
- [ ] **查询目的明确**（OLAP分析）
  - 海量离线处理 → 推荐 MaxCompute
  - OLAP 数仓分析 → 推荐 AnalyticDB MySQL
...
| 转化为下游开发 | 先生成 SQL → 调用 `/dq-assistant` 或 `/test-engineer` | [下游联动](#与下游-skill-的联动) |
...
> 📖 **完整最佳实践指南**：[references/best-practices.md](references/best-practices.md) — 包含 SQL 写法正反例、ADB vs MaxCompute 差异、性能调优。

### 0. 速查卡片

| 卡片 | 核心原则 |
|------|---------|
| 🎯 明确方言 | 写 SQL 前必须确定是 ADB MySQL 还是 MaxCompute |
| 🚫 避免 SELECT * | 只查需要的字段 |
| 📊 优先 CTE | 复杂查询用 CTE 代替嵌套子查询 |
| 📅 分区裁剪必带 | WHERE 必须带分区字段 |
| 🔗 大表 JOIN 必优化 | 关联键必须是分布键或使用 MAPJOIN |
| ❓ NULL 处理明确 | 关键字段用 `COALESCE/NVL` |
| 📝 注释完整 | 关键 SQL 必带作者、目的、修改时间 |
| 🧪 测试覆盖 | 复杂 SQL 必须有测试用例和性能基线 |
...
示例：
"使用AnalyticDB MySQL，统计2024年Q1各区域各品类的销售额和订单量，
...
```bash
# 1. 生成核心查询
/sql-gen 统计近30天每日销售额、订单量、客单价，使用AnalyticDB MySQL
...
- ✅ SQL审查器 (sql-reviewer)
- ✅ 执行计划分析器 (sql-explain)
- ✅ 多数据库支持 (MaxCompute/AnalyticDB MySQL)
