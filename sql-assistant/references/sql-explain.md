...
本套件仅支持阿里云 **AnalyticDB MySQL** 和 **MaxCompute**。其他数据库（MySQL 原生、PostgreSQL、SQL Server、Oracle 等）不在支持范围内。

### AnalyticDB MySQL
- `EXPLAIN <SQL>` — 查看逻辑执行计划
- `EXPLAIN ANALYZE <SQL>` — 实际执行并返回运行时统计（行数、耗时）
- `EXPLAIN FORMAT=json <SQL>` — JSON 格式执行计划，便于程序解析
- 关注点：分布方式（Hash/Broadcast）、分区裁剪是否命中、聚集索引是否生效、Stage/Operator 层级

### MaxCompute
- `EXPLAIN <SQL>` — 查看 MaxCompute 作业的执行计划（Map/Reduce/Join 阶段）
- 通过 **LogView** 链接查看完整的 Job 运行详情（Fuxi 作业、输入/输出量、耗时）
- 关注点：Mapper/Reducer 数量、数据倾斜、JOIN 方式（HashJoin vs MapJoin）、分区裁剪

### 获取执行计划的命令
```sql
-- ADB MySQL
EXPLAIN ANALYZE SELECT order_id, SUM(amount) FROM dwd_trade_order
WHERE dt >= '2024-01-01' AND dt < '2024-02-01' GROUP BY order_id;

-- MaxCompute（在 DataWorks 或客户端执行）
EXPLAIN SELECT user_id, COUNT(*) FROM ods_page_view
WHERE pt = '202401' GROUP BY user_id;
```
...
```sql
-- 建议创建的索引（⚠️ ADB不支持独立CREATE INDEX，须内联INDEX(col)或ALTER TABLE ADD INDEX(col)）
-- ALTER TABLE ... ADD INDEX idx_name(col);
```
